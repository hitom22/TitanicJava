...
public class Exercise_RadioButtonItemEvent extends JFrame {
	...
	public Exercise_RadioButtonItemEvent() {
		...
		JPanel radioPanel = new JPanel(); // 라디오 버튼을 추가할 팬을 만듦.
			...
			fruits[i].addItemListener(new ItemListener() {
				...
				public void itemStateChanged(ItemEvent e) {
					...
					if(fruits[0].isSelected()) {
						imageLabel.setIcon(images[0]);  // setIcon() 메소드 처음봄, Label 이미지를 Set해주는 메소드
		...
		c.add(radioPanel, BorderLayout.NORTH);   // 라디오팬을 컨텐트팬에 추가함. (탑으로 붙여서 보낼거임)
		c.add(imageLabel, BorderLayout.CENTER);  // 이미지 라벨은 센터에 위치함
		imageLabel.setHorizontalAlignment(SwingConstants.CENTER);  // 이미지 라벨안에 이미지도 가운데로 정렬
