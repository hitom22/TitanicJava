			...
			fruits[i].addItemListener(new ItemListener() {	// 아이템리스너 핸들러 등록
				...
				public void itemStateChanged(ItemEvent e) {  // 아이템 선택될때 
					if(e.getStateChange() == ItemEvent.SELECTED) {  // 선택이 됐으면
						if(e.getItem().equals(fruits[0])) {  // 그 이름이 사과인가?
							sum += 100;						// 그럼 100을 더하시오
						}else if(e.getItem().equals(fruits[1])) { // 그 이름이 배인가?
							sum += 500;						// 그럼 500을 더하시오
						}else if(e.getItem().equals(fruits[2])) { // 그이름이 체리인가?
							sum += 2000;					// 그럼 2000을 더하시오
						}
					}else { // 체크가 해제되면
						if(e.getItem().equals(fruits[0])) { // 사과는 100빼고
							...
							sum -= 500;
						}else if(e.getItem().equals(fruits[2])) { // 체리는 2000빼시오
							sum -= 2000;
						}
					}
